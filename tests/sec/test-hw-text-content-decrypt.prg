...
m.XML.Load("test-hw-text-content-encrypt.xml")
...
* decrypt the key (we must know / have the private key, of course)
m.KeyContext = m.DecKeyInfo.EncryptedCtx
m.DecKeyInfo.LoadKey("alice-privkey.pem", .T.)
...
m.XML.Save("test-hw-text-content-decrypt.xml")
