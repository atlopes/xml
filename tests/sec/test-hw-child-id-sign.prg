...
* add a reference to the signed object (in this case, a segment of the document with its own Id)
m.Segment = m.XML.selectNodes("//Child").item(0)
* will be siged with its original Id
m.Options = CREATEOBJECT("Collection")
m.Options.Add(.F., "OverwriteId")
m.DSig.AddReference(m.Segment, HASH_SHA1, "http://www.w3.org/2000/09/xmldsig#enveloped-signature", m.Options)
...
m.XML.Save("test-hw-child-id-sign.xml")
MESSAGEBOX(m.XML.xml)
